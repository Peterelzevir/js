...
//setting disini brok
const bot = new Telegraf('7339346205:AAEMBKKHkvPXNp-umZcx1mDjYamGyjPezVA');
const adminId = '7443341991';  // Ganti dengan ID admin bot

// Fungsi untuk memuat sesi pengguna dari file JSON
const loadUserSession = (userId) => {
  const filePath = path.join(__dirname, `sessionMINTSSZ_${userId}.json`);
...
  const filePath = path.join(__dirname, 'premium_usersMINTSSZ.json');
  if (fs.existsSync(filePath)) {
    return JSON.parse(fs.readFileSync(filePath));
  }
  return {};
...
const sendNoAccessMessage = (ctx) => {
  ...
  const message = `🖐🏻 Halo @${username} Saya adalah Bot Convert File By [MINTSSZ](https://t.me/MINTSWS)\n\nUntuk dapat akses fitur bot silahkan hubungi @MINTSWS`;
  ctx.replyWithMarkdown(message, {
    reply_markup: {
      inline_keyboard: [
        [{ text: '📩 Order', url: 'https://t.me/MINTSWS' }]
      ]
    }
  });
};
