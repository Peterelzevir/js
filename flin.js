...
//telegram @hiyaok
//harga script Rp.1.500.000
//BUTUH JASA BOT TELEGRAM? CHAT TELE : @hiyaok
...
//setting disini brok
const bot = new Telegraf('7230985866:AAFTyTns6JIray7szAb-mIq_-zbIFVY74Mg');
const adminId = '1050245401';  // Ganti dengan ID admin bot

// Fungsi untuk memuat sesi pengguna dari file JSON
const loadUserSession = (userId) => {
  const filePath = path.join(__dirname, `sessionFliny_${userId}.json`);
...
  const filePath = path.join(__dirname, 'premium_usersFliny.json');
  if (fs.existsSync(filePath)) {
    return JSON.parse(fs.readFileSync(filePath));
  }
  return {};
...
const sendNoAccessMessage = (ctx) => {
  ...
  const message = `🖐🏻 Halo @${username} Saya adalah Bot Convert File By [Fliny](https://t.me/davidartha)\n\nUntuk dapat akses fitur bot silahkan hubungi @davidartha`;
  ctx.replyWithMarkdown(message, {
    reply_markup: {
      inline_keyboard: [
        [{ text: '📩 Order', url: 'https://t.me/davidartha' }]
      ]
    }
  });
};
