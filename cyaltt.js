...
const bot = new Telegraf('6487390784:AAFlg7vE4hH6WB6cvPqSvtkGljvDHHROiFs');
const adminId = '5530482962';  // Ganti dengan ID admin bot
...
  const filePath = path.join(__dirname, 'premiumcyaltt_users.json');
  if (fs.existsSync(filePath)) {
    return JSON.parse(fs.readFileSync(filePath));
  }
  return {};
...
const sendNoAccessMessage = (ctx) => {
  ...
  const message = `🖐🏻 Halo @${username} Saya adalah Bot Convert File By [cyaaalt](https://t.me/cyaltt)\n\nUntuk dapat akses fitur bot silahkan hubungi @cyaltt`;
  ctx.replyWithMarkdown(message, {
    reply_markup: {
      inline_keyboard: [
        [{ text: 'Order Sewa Bot 💡', url: 'https://t.me/cyaltt' }]
      ]
    }
  });
};
